...
-   $f(\cdot)$: **unknown** function^[$f(\cdot)$ is assumed fixed from frequentist point of view; $f(\cdot)$ is random in the Bayesian framework.] describing the relationship between $X$ and the **mean of** $Y$. <!-- - $f$ represents the systematic information that X provides about Y. -->
...
::: columns
::: {.column width="25%"}
-  $\text{MSE}_{\texttt{Tr}}$ **([gray]{style="color:gray"})** is decreasing with the complexity.
-  $\text{MSE}_{\texttt{Te}}$ **([red]{style="color:red"})** is **U-shaped**: goes down then up with the complexity.
...
- **Overfitting**: **[Low bias and High variance]{.blue}**

- **Underfitting**: **[High bias and Low variance]{.blue}**
