...
- [**Subset Selection**.]{.green} Identify a subset of all the candidate predictors with the *best* OLS performance.
...
- [**Shrinkage**.]{.green} Fit a model that forces some coefficients to be shrunk to zero.
  + *Lasso* `glmnet::glmnet()`

. . .

- [**Dimension Reduction**.]{.green} Find $m$ representative features that are linear combinations of the $p$ original predictors ($m \ll p$), then fit least squares. 
...
```{r fig.dim = c(12, 6), echo=c(2, 3), out.width = '100%'}
#| code-fold: true
...
- $\by = \bX' \boldsymbol \beta + \epsilon = \sum_{j = 1}^p X_j \times 0.4^{\sqrt{j}} + \epsilon$
...
```{r}
    ...
    plot(colMeans(betamat_1 != 0), type = "b", pch = 15, 
         xlab = "Predictor Index", ylab = "Selection Proportion",
         ylim = c(0, 1.1))
...
plot(beta, beta, xlab = "beta", ylab = "Cofficient estimates", 
