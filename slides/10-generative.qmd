...
- We assume predictors are *fixed* and *known*, and we care about the distribution of the response conditional on the predictors, $p(y \mid \bx)$
...
- If the distribution of the predictors is approximately _normal_ in each of the classes and the _sample size is small_, the generative models could perform better and more stable again.
...
- To infer $p(y \mid \bx)$ from a generative model, Bayes theorem comes into play:
...
Different generative models use different priors and likelihood functions, resulting in different strengths and weakness.
:::
...
- Centroids: $\widehat{\bmu}_k = n_k^{-1} \sum_{i: \,y_i = k} \bx_i$
...
- $(\bx - \bmu_k)' \Sigma^{-1} (\bx - \bmu_k)$ is the **Mahalanobis distance** between $\bx$ and the centroid $\bmu_k$.
  + [Classify $y$ so that its $\bx$ and the centroid of the labeled class is the closest.]{.green}

$$\begin{align}
& - \frac{1}{2} (\bx - \bmu_k)' \Sigma^{-1} (\bx - \bmu_k) + \log(\pi_k)\\
&=~ \bx' \underbrace{\Sigma^{-1} \bmu_k}_{\bw_k} \underbrace{- \frac{1}{2}\bmu_k' \Sigma^{-1} \bmu_k + \log(\pi_k)}_{b_k} + \text{irrelevant terms} \\
&=~ \bx' \bw_k + b_k + \text{irrelevant terms}\\
&=~ \delta_k(\bx) + \text{irrelevant terms}
\end{align}$$
...
table(logit_pred > 0.5, Default$default)
```
...
- Logistic regression training accuracy is `r round(sum(diag(table(logit_pred > 0.5, Default$default)))/nrow(Default), 3)`

- LDA training accuracy is `r round(sum(diag(table(lda_pred$class, Default$default)))/nrow(Default), 3)`
...
table(qda_pred$class, Default$default)
```

- QDA training accuracy is `r round(sum(diag(table(qda_pred$class, Default$default)))/nrow(Default), 3)`
...
- [**NB assumption for $f_k(\bx)$**]{.green}: 
  + features are [independent]{.green} in each class!
...
- Condsidering the *joint distribution* of predictors is a pain when $p$ is large.

- We don't believe $p$ predictors are not associated at all, but 

this "naive" assumption leads to decent results when *$n$ is not large enough relative to $p$*.
...
```{r}
#| echo: false
qda_fit <- MASS::lda(default ~ balance, data = Default)
qda_pred <- predict(qda_fit, data = Default)
table(qda_pred$class, Default$default)
```
...
table(nb_pred, Default$default)
```


- Naive Bayes training accuracy is `r round(sum(diag(table(nb_pred, Default$default)))/nrow(Default), 3)`
...
The log odds is _quadratic_ in $\bx$, where $a_k$, $c_{kj}$ and $d_{kjl}$ are functions of $\pi_k$, $\pi_K$, $\bmu_k$, $\bmu_K$, $\Sigma_k$ and $\Sigma_K$.
...
The log odds takes the form of a _generalized additive model_!
...
- [_Neither QDA nor NB is a special case of the other_]{.green}
  + NB is more _flexible_ for any transformation of $x_j$, $g_{kj}(x_j)$.
  + NB is more _restrictive_ because of its pure _additive_ fit with no quadratic interactions $d_{kjl}x_jx_l$.
...
- LDA tends to be better than QDA if $n$ is small, and so reducing variance is crucial.

- QDA is recommended if $n$ is large so that the variance of the classifier is not a major concern.
