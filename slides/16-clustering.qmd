...
- Seek a $C(\cdot)$ that minimizes the **within-cluster distance** $W(C)$


$$ W(C) = \frac{1}{2}\sum_{k=1}^K\sum_{(i, i'): C(i), C(i')=k} d(\bx_i, \bx_{i'})$$
...
$$ \underset{C, \, \{m_k\}_{k=1}^K} \min \sum_{k=1}^K \sum_{C(i) = k} \lVert \bx_i - m_k \rVert^2, $$
...
::: {.column width="50%"}
**Given $C(\cdot)$, compute the cluster centroid $m_k$**
...
**Given $m_k$, do new assignment $C(\cdot)$**
...
```{r}
#| echo: false
#| fig-cap: "Source: ISL Figure 12.9"
...
&=\sum_{i=1}^n \E \left[ \log \left[ \prod_{k=1}^K p_kp(\bx_i \mid \btheta)^{I(z_i = k)}\right]\right]\\
&=\sum_{i=1}^n \sum_{k=1}^K \E (I(z_i=k)) \log \left[p_kp(\bx_i \mid \btheta) \right]\\
&=\sum_{i=1}^n \sum_{k=1}^K p\left(z_i = k \mid \bx_i, \btheta \right) \log \left[p_kp(\bx_i \mid \btheta) \right]\\
&=\sum_{i=1}^n \sum_{k=1}^K r_{ik} \log p_k + \sum_{i=1}^n \sum_{k=1}^K r_{ik}  \log p(\bx_i \mid \btheta)\end{align}$$


## EM for GMM
::: {.midi}
$\begin{align} Q\left(\btheta, \btheta^{(t-1)} \right)=\sum_{i=1}^n \sum_{k=1}^K r_{ik} \log p_k + \sum_{i=1}^n \sum_{k=1}^K r_{ik}  \log p(\bx_i \mid \btheta)\end{align}$
:::
