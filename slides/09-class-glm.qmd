---
title: 'Generalized Linear Models `r fontawesome::fa("0")` `r fontawesome::fa("1")`'
...
$$ y_i \mid x_i \stackrel{indep}{\sim} \text{Bernoulli}(\pi = \pi_i = \pi(x_i)) = \text{binomial}(m=1,\pi = \pi_i) $$
...
round(summ_logit_fit$coefficients, 4)
```
...
$\sqrt{(1 - \text{TPR})^2 + (1 - \text{TNR})^2}$ ([the distance to the optimal point]{.green})
...
```{r}
#| echo: true
multino_fit <- nnet::multinom(prog2 ~ ses + write, 
                              data = multino_data)
...
```{r, echo=FALSE, out.width="100%", fig.asp=0.55}
...
## Softmax Coding^[The fitted values, predictions, log odds between any pair of classes, and other key model outputs remain the same.]
...
- Repeated measures and binomial logistic regression (MSSC 5780)
