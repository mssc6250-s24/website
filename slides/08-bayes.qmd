---
title: 'Bayesian Linear Regression `r fontawesome::fa("scale-unbalanced")`'
...
2. [An election is coming up and a pollster claims that candidate A has a 0.9 probability of winning. How do you interpret this probability?]{.green}
...
3. [Two claims.]{.green}
    ...
    [(1) Ben claims he can predict the coin flip outcome. To test his claim, you flip a fair coin 5 times and he correctly predicts all.]{.green} 
    
    [(2) Emily claims she can distinguish natural and artificial sweeteners. To test her claim, you give her 5 samples and she correctly identifies each.]{.green} 
    
    [In light of these experiments, what do you conclude?]{.green}

    a. You're more confident in Emily's claim than Ben's claim.
    b. The evidence supporting Ben's claim is just as strong as the evidence supporting Emily's claim.
...
- The *frequentist* philosophy interprets probability as the [*long-run* relative frequency of a *repeatable* experiment]{.green}.

. . .

- The probability that some outcome of a process will be obtained is defined as the **relative frequency** with which that outcome would be obtained _if the process were repeated **a large number of times** independently under **similar conditions**._
...
- For the statement "_candidate A has a 0.9 probability of winning_"
  + A frequentist might 
    i. say the conclusion is wrong
    ii. weirdly say in long-run *hypothetical* repetitions of the election, candidate A would win roughly 90% of the time.
...
- In Question 3, in a frequentist analysis, "5 out of 5" is "5 out of 5" no matter if it's in the context of Ben's coins or Emily's sweeteners.
...
- Ben probably overstates his ability but Emily's claim is reasonable, right?
...
::: {.column width="80%"}
- [*We are not stubborn!*]{.green} If Ben had correctly predicted the outcome of *1 million* coin flips, the strength of this data would far surpass that of our prior, leading to a posterior conclusion that perhaps Ben is psychic!
:::
...
- A Bayesian answer (a) *what's the chance that I actually have COVID?*
- A frequentist answer (b) *if in fact I do not have COVID, what's the chance that I would’ve gotten this positive test result?*
:::
...
- [**Prior info**:]{.green} Our prior information suggested that 40% of the articles are fake, 
...
- [**Data come in**:]{.green} The usage of an `!` might seem odd for a real article. The exclamation point data is more consistent with fake news.
...
- *Opposite position*: We know the incoming article used `!` (data); we don't know whether or not the article is fake (what we want to decide).
...
- Our posterior understanding evolved quite a bit – the chance that the article is fake jumped to 88.9%.
...
- $L(\theta \mid y_1,\dots, y_n)$: the likelihood function of $\theta$ given observed data $\by = \{y_i \}_{i = 1}^n$. 
...
- According to previous 30 polls, Michelle's support is centered round 45%, and she polled at around 35% in the dreariest days and around 55% in the best days.
...
- We just assign priors to the unknown parameters, then do the posterior inference using Bayes' rule!
...
## Capital Bikeshare `bikes` Data in the *Washington, D.C.* Area.
