...
- Find a **biased** estimator $\hat{\bbeta}$ that has *smaller variance and MSE* than the LSE ${\bf b}$.
...
plot(x, dnorm(x), type = "l", lwd = 3, axes = F, xlab = "", ylab = "",
     xlim = c(-8, 8))
title(main = expression(paste("Sampling distribution a biased estimator ", hat(beta))))
axis(1, at = c(-8, -1, 0, 8), 
...
text(2.5, dnorm(1.5), paste("Var(b) large"))
```
:::
::::
...
- Has *less* degrees of freedom in the sense that _the cost increases when large coefficients are used_.
...
contour(beta1, beta2, pen, levels = quantile(pen, quanlvl), las = 1)
points(1, 1, pch = 19, col = "red", cex = 2)
box()
```
...
- The GCV criterion is given by $$\text{GCV}(\lambda) = \frac{1}{n}\sum_{i=1}^n \left[ \frac{y_i - x_i' \widehat{\bbeta}^\text{r}_\lambda}{1 - \Trace(\bS_\lambda)/n} \right]^2$$
