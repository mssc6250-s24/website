...
- The training points lied on the dashed lines are **support vectors**:
  + if they were moved, then the maximal margin hyperplane would move too.
...
\text{subject to} \quad & \frac{1}{\lVert \bbeta \lVert} y_i(\bx' \bbeta + \beta_0) \geq M, \,\, i = 1, \ldots, n.
\end{align}

- The constraint requires that each point be on the correct side of the hyperplane, with some cushion.

- Note that the scale of $\bbeta$ can be arbitrary, so just set it as $\lVert \bbeta \rVert = 1$:
...
- The maximal margin hyperplane is extremely sensitive to a change in a single observation: it may overfit the training data.
...
- Allow some observations to be on the incorrect side of the margin, or even the incorrect side of the hyperplane (training points misclassified by the classifier).
...
- $\epsilon_1, \dots, \epsilon_n$ are **slack variables** that allow individual observations to be on slack the wrong side of the margin or the hyperplane.
...
to the margin/hyperplane that we will tolerate.
  + $B = 0$: no budget for violations ($\epsilon_1 = \cdots = \epsilon_n = 0$)
  + $B > 0$: no more than $B$ observations can be on the wrong side of the hyperplane. ($\epsilon_i > 1$)
...
\begin{align}
\text{min} \quad & \frac{1}{2}\lVert \bbeta \rVert^2 + C \sum_{i=1}^n \epsilon_i \\
\text{s.t} \quad & y_i (\bx_i' \bbeta + \beta_0) \geq (1 - \epsilon_i), \\
\text{and} \quad & \epsilon_i \geq 0, \,\, i = 1, \ldots, n,
\end{align}
:::
...
- SVM decision rule is based only on a small subset of the training observations (robust to the behavior of observations that are far away from the hyperplane.)

- LDA classification rule depends on the mean of _all_ of the observations within each class, and within-class covariance matrix computed using *all* of the observations.

- Logistic regression, unlike LDA, has very low sensitivity to observations far from the decision boundary too.
:::
:::
...
- In SVM, we could address non-linear boundaries by enlarging the feature space.
...
$$f(\bx) = \beta_0 + \sum_{i=1}^n\alpha_i\langle \bx, \bx_{i} \rangle$$
...
- **Linear kernel**: $K\left(\bx_0, \bx_{i}\right) = \langle \bx_0, \bx_{i'} \rangle = \sum_{j=1}^px_{0j}x_{i'j}$

- **Polynomial kernel**: $K\left(\bx_0, \bx_{i}\right) = \left(1 + \sum_{j=1}^px_{0j}x_{i'j}\right)^d$

- **Radial kernel**: $K\left(\bx_0, \bx_{i'}\right) = \exp \left(-\gamma\sum_{j=1}^p (x_{0j}-x_{i'j})^2 \right)$
