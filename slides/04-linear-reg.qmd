...
- It assumes that the dependence of $Y$ on $X_1, X_2, \dots, X_k$^[The textbook uses $p$ instead of $k$ to denote the number of predictors in the model. We use $k$ here, but switch to $p$ starting next week.] is linear.
...
::: {.column width="33%"}
```{r}
knitr::include_graphics("./images/04-linear-reg/yt.svg")
```
:::

::: {.column width="33%"}
```{r}
knitr::include_graphics("./images/04-linear-reg/fb.jpeg")
```
:::

::: {.column width="33%"}
```{r}
knitr::include_graphics("./images/04-linear-reg/ig.jpeg")
```
:::
...
knitr::include_graphics("./images/04-linear-reg//mlr_depress.jpeg")
```
...
```{r}
#| out-width: "60%"
knitr::include_graphics("./images/04-linear-reg/icecream.jpeg")
```
...
```{r}
#| out-width: "60%"
knitr::include_graphics("./images/04-linear-reg/drowning.jpeg")
```
...
```{r}
#| out-width: "100%"
knitr::include_graphics("./images/04-linear-reg/anova_mlr.png")
```
...
```{r}
#| out-width: "100%"
knitr::include_graphics("./images/04-linear-reg/normal_eqn.png")
```
...
#| out-width: "100%"
knitr::include_graphics("./images/04-linear-reg/ols_geom.png")
```
:::
::::
...
- $\ell(\bbeta)$, in machine learning, is called the (squared) **loss function**.
...
- Start with an initial value of $\bbeta$, say $\widehat{\bbeta} = (0.3, 1.5)$, we compute the $SS_{res}$
...
#| out-width: 70%
  ...
  plot(x, x^2, type = "l", col = "deepskyblue", lwd = 1.5,
       main = expression(paste("convex function ", f(x) == x^2)))
  points(0, 0, col = "red", pch = 19, cex = 2.5)
  
  # non-convex case
  x = seq(-4, 2, 0.01)
  plot(x, x^4 + 2*x^3 - 5*x^2, type = "l", col = "deepskyblue", lwd = 1.5,
       main = expression(paste("nonconvex function ", f(x) == x^4 + 2*x^3 - 5*x^2)))
...
- Whether $\nabla f(\bx)$ is positive or negative, we can always find a new point $\bx^\text{new}$ that makes $\nabla f(\bx) (\bx^\text{new} - \bx)$ less than 0, so that $\bx^\text{new}$ has a smaller functional value than $\bx$. 
...
- Stopping criterion:
  ...
  - (Relative) change of distance: $\lVert \bx^{(i)} - \bx^{(i-1)} \rVert / \lVert \bx^{(i-1)}\rVert< \epsilon$ or $\lVert \bx^{(i)} - \bx^{(i-1)} \rVert < \epsilon$
  - (Relative) change of functional value: $| f(\bx^{(i)}) - f(\bx^{(i-1)})| / |f(\bx^{(i)})| < \epsilon$ or $| f(\bx^{(i)}) - f(\bx^{(i-1)})| < \epsilon$
...
:::{style="font-size: 1.3em;"}
When $\bH$ or $\bH^{-1}$ is difficult to compute
